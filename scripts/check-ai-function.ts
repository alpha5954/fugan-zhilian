...

{
  ...
  check('给了语言要求（长句拆开）', SYSTEM_PROMPT.includes('超过 40 字就拆开'))
}
...

{
  ...
  // ---- ★ 「别当复读机」那一组 ----
  //
  // 这一组是用户反馈之后加的。原先的回答长这样：
  //
  //   数据里没有「下周是否加量」的直接结论。本周训练 7 天、建议每周 5 天……
  //   是否加量需由康复师判断。
  //
  // 不是回答，是**数据复读加一句免责声明**。根因是我上一轮加的那段
  // 「回答的语气」把它固化成了模板 —— 从一个样本里学过了头。
  //
  // 所以这几条断言的是**方向**：要求它给建议，且禁止那句默认结尾。
  check(
    '要求给出训练层面的建议',
    ASK_SYSTEM_PROMPT.includes('训练层面建议就直接给'),
    '不要求的话它会退化成数据复读',
  )
  check(
    '明确说了「不要因为严谨就什么都不说」',
    ASK_SYSTEM_PROMPT.includes('不要因为"严谨"就什么都不说'),
  )
  check(
    '把 decline 的范围收窄到医疗层面',
    ASK_SYSTEM_PROMPT.includes('只有医疗层面的问题'),
    '不收窄的话"该不该加量"也会被推给医生',
  )
  check(
    '明确排除了训练类问题（这些不该走 decline）',
    ASK_SYSTEM_PROMPT.includes('都不是**医疗问题'),
  )
  check(
    '★ 禁止把「需由医生判断」当默认结尾',
    ASK_SYSTEM_PROMPT.includes('不能当默认结尾'),
    '这是用户看到的那句免责声明',
  )
  // ★ 反向断言：上一轮那段善后模板必须**不在了**
  check(
    '★ 旧的「先如实说没有…最后说明该由谁判断」模板已删除',
    !ASK_SYSTEM_PROMPT.includes('最后说明这件事该由谁判断'),
    '留着的话它又会长成每次回答的固定收尾',
  )
  check(
    '给了「先结论后理由」的要求',
    ASK_SYSTEM_PROMPT.includes('先把结论说在前面'),
  )
  check('给了语言要求（长句拆开）', ASK_SYSTEM_PROMPT.includes('超过 40 字就拆开'))
}
