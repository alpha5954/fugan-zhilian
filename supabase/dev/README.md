...
> 端到端测试改一个跑一个即可，`e2e_care_links.py` 最慢（要建 3 个账号）。

## 端到端测试发现过的真实缺陷

`e2e_care_links.py` 抓到一个只在真实环境暴露的问题：

```
家属可以把自己的 care_links.relation 从 'family' 改成 'therapist'
```

根因是权限只约束到「行」这一层——RLS 限定了*能改哪一行*，触发器只拦了 `status` 字段，`relation` 这一列无人看管。本地测试当时没覆盖到，因为**断言的粒度是按字段写的，而漏洞恰好在一个没写断言的字段上**。

已由迁移 6 用列级授权修复（可更新列收窄到只有 `status`）。
