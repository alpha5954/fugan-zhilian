...
npm run check:insight      # 恢复评分、风险分级与建议生成（85 项）
npm run check:findings     # 评估结论：合规措辞、排序、与历史对比（35 项）
```

七个脚本都用 `node` 直接执行 `.ts`，靠的是 Node 22.6+ 的类型剥离，不需要额外装测试框架。
...
| 最严重的预警 | 系数 |
|---|---|
| critical（温度越阈值、信号中断） | ×0.6 |
| warning（接近阈值、识别异常） | ×0.85 |
...
**但那个修法是治标**：它承认了"分数 92 但有安全风险"这个状态是合理的，
只是把矛盾从视觉上藏起来。家属看到 92 分和一个红角标，记住的仍是 92。

**现在**：安全事件直接扣分（见上面「安全下调」），分数本身就把安全算了进去。
两个轴合并成一个 —— 分数高就不可能同时有红灯。`scoreBand` 和 `band` 都从
调整后的分数推出，**必然一致**，自检里有一条断言钉住这一点：
哪天有人把安全惩罚拿掉，那条断言会立刻变红。
...
```
┌─────────────────────────┐
│  概览  实时监测  预警  更多  │   ← 固定底栏，56px + 安全区
└─────────────────────────┘
```

| 位置 | 项 | 为什么 |
|---|---|---|
| 底栏 | 概览 / 实时监测 / 预警 | 打开就想知道的；预警带未处理角标 |
| 抽屉 | 康复评估 / 数据分析 / 我的设备 / 监护管理 / 关于 | 不是"每次打开都要看"的 |
