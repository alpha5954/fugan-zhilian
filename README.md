...
波形在深底上最接近示波器，长时间盯着也更省眼睛。做成**监测页内的开关**，
不做全站深色主题。三条理由：

1. 全站深色要维护两套完整配色，而医疗界面在明亮环境下用浅色更合适
2. 跳转时页面一黑一白会闪
3. 家属在户外用手机看，深色屏反光更难看

作用域是监测页的根元素，**顶栏不受影响** —— 那是应用外壳，不是仪器面板。
开关状态记在 localStorage。
...
`element.css` 里的映射写在 `:root` 上，形如 `--el-bg-color: var(--surface)`。
而 **CSS 自定义属性里的 `var()` 是在声明处求值的** —— `:root` 上 `--surface`
是 `#ffffff`，于是 `--el-bg-color` 一路继承下去的就是 `#ffffff` 这个**结果**，
不是"对 `--surface` 的引用"。

所以只覆盖 `--surface` 是不够的，EP 的按钮、标签、警告条**仍然全白**。
实测确认过：作用域内 `--surface` 已经是 `#111a24`，而按钮拿到的
`--el-bg-color` 还是 `#ffffff`。

**② 主色的 light-N 变体要跟着一起重声明，否则会出现"深蓝底 + 深色字"。**

`--el-color-primary` 同样停在根上的 `#1a4d87`，而 `--el-color-white`
已经跟着 `--surface` 变成了深色 —— 主按钮直接读不出来。实测踩过。
另外 `--el-color-white` 在深色作用域里**要设成字面意义的白**，
它是"彩色底上的文字色"，跟着 `--surface` 走会变成深色字。

#### 图表够不着页面作用域

`token()` 从 `document.documentElement` 读，**读不到页面级作用域里的覆盖值**。
所以 `chartTheme.ts` 里另有一套写死的暗色轴色，两边必须配套改 ——
这一点在两个文件里都标了。

好消息是范围可控：只有监测页会切暗色，而监测页只用 `SignalChart`，
所以只有这一个图表组件需要 `dark` 属性。
