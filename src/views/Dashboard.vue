...
<template>
  <div class="home">
    ...
    <!-- ================= AI 深度分析 =================
         ...
         ⚠️ 两页**都默认展开**（这条曾经是折叠的，2026-09-24 改回来）。

            当初折叠的理由写的是"避免误触计费"—— **那个理由站不住**：
            展开只是把「用 AI 深入分析」那个按钮显示出来，**分析仍然是
            点了才跑**。展开本身不发任何请求、不花一分钱（挂载时的
            ai.restore 只读本地缓存）。当时把"展开"和"触发"混为一谈了。

            折叠真正的代价是：功能藏在一个要点的箭头后面，而没有人会去点
            一个自己不知道存在的按钮。
