 ...
 * 所以进入热敷场景后，第一段过热保证在下面这个区间内出现一次。
 *
 * ⚠️ **这个时刻不能太早。** 第一版排在 25~55 秒，结果整条曲线读起来是
 *    **先慢后快** —— 和场景说明里"真实升温先快后慢"正好相反。实测：
 *
 *      0-20 秒 涨 2.60°C（正常升温，先快后慢）
 *      20-40 秒 涨 1.16°C（还在趋近 45）
 *      40-60 秒 涨 5.01°C（过热启动，猛冲 55）← 把形状顶翻了
 *
 *    热敷升温的时间常数是 28 秒，要走完约 90 秒才看得出"趋近"的样子。
 *    所以第一段过热必须排在**正常段走完之后**，用户的抱怨就是这条：
 *    "局部温度的模拟数据是先慢后快"。
 *
 ...
 * 不要把这一段当成真实发生率。
 */
const FIRST_OVERHEAT_MIN_MS = 90_000
const FIRST_OVERHEAT_MAX_MS = 120_000
...
> = {
  ...
  hotpack: {
    label: '热敷监测',
    detail:
      `肢体静止，局部皮温向热敷袋温度指数趋近（先快后慢，不是匀速），` +
      `正常稳定在 45 °C 上下；热敷袋加热过久时会冲到 55 °C 上下，` +
      `越过 ${TEMP_ALERT_THRESHOLD} °C 触发预警`, 
  },
}
