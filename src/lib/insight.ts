 ...
 * 说什么的必须是这张表。
 *
 * 【2026-09-23 补：大模型已经接上了，这张表没变】
 * 上面那段仍然成立，而且现在有了一个具体的例子。接的是 **DeepSeek 深度
 * 分析**（lib/aiContext.ts 组装输入、lib/aiReply.ts 校验输出），它：
 *
 *   · 是一个**并列模块**，不在这张表里面，也不改这张表的任何一条；
 *   · 界面上有独立外壳和「AI 生成」标签，不与 FindingCard 混排；
 *   · 只拿得到本层算好的结论，拿不到原始记录；
 *   · 不能凭空造数值 —— 它写出的每个数字都必须在本层给出的数据里出现过，
 *     否则那一条直接被丢掉（见 aiReply.ts 的数值闸门）。
 *
 * 所以分工是：**这张表决定"说什么"，AI 决定"还能怎么解读"。** 前者免费
 * 且始终显示，后者是可选项。答辩被问到"评估用的是什么模型"时，答案仍然是
 * "评估结论用规则，AI 只做解读" —— 这句话没有变。
 */
function adviceFor(
  parts: ScorePart[],
  weekAlerts: Alert[],
